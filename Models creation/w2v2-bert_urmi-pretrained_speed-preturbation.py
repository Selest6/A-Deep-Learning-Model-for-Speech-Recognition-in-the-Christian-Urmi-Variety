...
def run_finetune_speed_perturbation(args, finetune_root: Path, encoder_ckpt_dir: Path) -> None:
    ft_args = argparse.Namespace(model_name_or_path=args.base_model_name_or_path, train_dir=str(Path(args.train_dir).resolve()), test_dir=str(Path(args.test_dir).resolve()), output_dir=str(finetune_root.resolve()), experiment_name='w2v2-bert_urmi-pretrained_speed-preturbation', num_train_epochs=30.0, per_device_train_batch_size=1, per_device_eval_batch_size=1, learning_rate=1.5e-05, warmup_ratio=0.1, lr_scheduler_type='cosine', weight_decay=0.01, max_steps=None, max_grad_norm=0.5, gradient_accumulation_steps=8, precision=args.precision, max_train_samples=None, save_steps=500, eval_steps=500, logging_steps=50, mask_time_prob=0.0, mask_feature_prob=0.0, train_augment_mode='speed', speed_factors='0.9,1.1', speed_prob=0.5, noise_prob=0.35, noise_snr_min_db=10.0, noise_snr_max_db=20.0, noise_gain_db_max=3.0, add_adapter=True, freeze_base_model=False, wandb_project=args.wandb_project, wandb_run_name='w2v2-bert_urmi-pretrained_speed-preturbation', disable_wandb=args.disable_wandb, seed=args.seed, save_total_limit=3, keep_intermediate_checkpoints=False, beam_width=10, skip_beam_eval=False, resume_from_checkpoint=getattr(args, 'finetune_resume_from_checkpoint', None), resume_best_search_dir=None, reset_optimizer_on_resume=False, resume_new_hparams_same_step=False, model_weights_override_from=None, copy_best_checkpoint_to=None, pretrained_encoder_dir=str(encoder_ckpt_dir.resolve()), ctc_zero_infinity=True, debug_trainable_grads=False)
...
def main() -> None:
    ...
    if args.phase in ('both', 'finetune_only'):
        ...
        run_finetune_speed_perturbation(args, finetune_root, encoder_ckpt)
        pipeline['finetune_output_dir'] = str(finetune_root.resolve())
        pipeline['encoder_checkpoint'] = str(encoder_ckpt.resolve())
    if args.phase in ('both', 'pretrain_only'):
