...
from transformers import Trainer, TrainerCallback, TrainingArguments, Wav2Vec2CTCTokenizer, Wav2Vec2ForCTC, Wav2Vec2Processor
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--max_steps', type=int, default=-1, help='Остановить обучение при достижении этого глобального шага (всего с начала рана). Если > 0, перекрывает num_train_epochs. Удобно при --resume_from_checkpoint, чтобы дойти до фиксированного шага (напр. как у другой модели).')
    p.add_argument('--per_device_train_batch_size', type=int, default=2, help='По умолчанию ниже для стабильности fp16.')
    p.add_argument('--per_device_eval_batch_size', type=int, default=2)
    p.add_argument('--learning_rate', type=float, default=2e-05, help='Полный fine-tune ~1B MMS: для малого корпуса и fp16 чуть ниже — меньше риска дрейфа/NaN (при быстрой сходимости можно попробовать 3e-5–4e-5). Для --train_adapters_only обычно на порядок выше.')
    p.add_argument('--warmup_steps', type=int, default=0, help='Warmup в шагах; если >0, warmup_ratio не передаётся в Trainer. По умолчанию 0: для MMS надёжнее warmup через --warmup_ratio (доля от всех шагов, подходит при разном размере данных). Фиксированные шаги (как в рецептах wav2vec2-bert) задавайте явно только если осознанно.')
    p.add_argument('--warmup_ratio', type=float, default=0.12, help='Доля warmup при warmup_steps==0 (~12%% первых шагов — чуть мягче старт градиентов для глубокой MMS).')
    p.add_argument('--lr_scheduler_type', type=str, default='cosine', help='Расписание LR (HF TrainingArguments): для длинного дообучения cosine часто плавнее linear.')
    p.add_argument('--weight_decay', type=float, default=0.01, help='AdamW weight decay (0 отключает).')
    p.add_argument('--mask_time_prob', type=float, default=0.0, help='SpecAugment по времени при обучении. Дефолт 0 для MMS полного FT на ограниченных данных (стабильнее сходимость; регуляризацию даёт уже weight_decay + cosine LR). При больших корпусах попробуйте 0.05 — как в типичных HF-рецептах; >0 требует достаточно длинных клипов (см. лог min WAV после старта).')
    p.add_argument('--gradient_accumulation_steps', type=int, default=2, help='Эффективный batch = × per_device_* . У больших моделей можно поднять до 4 при нехватке стабильности (уменьшит число шагов оптимизатора за эпоху; при падении качества добавьте эпохи или чуть LR).')
    ...
    p.add_argument('--enable_beam_eval', action='store_true', help='Включить финальную оценку beam search (pyctcdecode; долго на CPU). По умолчанию только greedy.')
    p.add_argument('--beam_logits_chunk_samples', type=int, default=256, help='Финальный beam считает полные logits; предсказания идут кусками по столько записей из train/test, чтобы не держать весь сплит в одном eval/predict на GPU/RAM (0 или отриц. — один проход целиком).')
    ...
    return p.parse_args()
RESUME_FROM_BEST_SENTINEL = '__resume_best__'
...
def parse_resume_from_checkpoint_arg(value: Optional[str]) -> Union[bool, str, None]:
    ...
    if v.lower() in ('last', 'true', '1', 'yes'):
        return True
    if v.lower() == 'best':
        return RESUME_FROM_BEST_SENTINEL
    return str(Path(v).expanduser())

def _load_trainer_state_dict(trainer_state_path: Path) -> Optional[Dict[str, Any]]:
    if not trainer_state_path.is_file():
        return None
    text = trainer_state_path.read_text(encoding='utf-8')
    text = re.sub(':\\s*NaN\\b', ': null', text)
    try:
        return json.loads(text)
    except json.JSONDecodeError:
        logger.warning('Не разобрать trainer_state.json: %s', trainer_state_path)
        return None

def find_best_checkpoint_dir(search_dir: Path) -> Optional[Path]:
    search_dir = search_dir.expanduser().resolve()
    latest = find_latest_checkpoint_dir(search_dir)
    if latest is None:
        logger.warning('find_best_checkpoint_dir: нет checkpoint-* в %s', search_dir)
        return None
    data = _load_trainer_state_dict(latest / 'trainer_state.json')
    if not data:
        return latest.resolve()
    rel = data.get('best_model_checkpoint')
    if not rel:
        return latest.resolve()
    name = Path(str(rel)).name
    cand = (search_dir / name).resolve()
    if cand.is_dir():
        bm = data.get('best_metric')
        bgs = data.get('best_global_step')
        logger.info('Лучший чекпоинт по eval_cer: %s (best_metric=%s, best_global_step=%s)', cand, bm, bgs)
        return cand
    logger.warning('best_model_checkpoint=%s не найден как каталог в %s — используем последний %s', rel, search_dir, latest)
    return latest.resolve()
...
def resolve_resume_checkpoint_dir(output_dir: Path, resume_parsed: Union[bool, str, None]) -> Optional[Path]:
    ...
    if resume_parsed == RESUME_FROM_BEST_SENTINEL:
        return find_best_checkpoint_dir(output_dir)
...
class CtcEvalLossCerCallback(TrainerCallback):

    def on_evaluate(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs: Any) -> None:
        m = kwargs.get('metrics') or {}
        loss = m.get('eval_loss')
        cer = m.get('eval_cer')
        wer = m.get('eval_wer')
        step = getattr(state, 'global_step', None)
        if loss is None and cer is None:
            return
        logger.info('CTC eval @ step %s: eval_loss=%s eval_cer=%s eval_wer=%s', step, f'{loss:.6f}' if loss is not None else 'n/a', f'{cer:.6f}' if cer is not None else 'n/a', f'{wer:.6f}' if wer is not None else 'n/a')
        if cer is not None:
            try:
                cer_f = float(cer)
            except (TypeError, ValueError):
                cer_f = None
            if cer_f is not None and cer_f >= 0.999 and (loss is not None):
                logger.warning('CTC eval: CER≈1 при eval_loss=%s — проверьте greedy-декод (часто пустой pred / коллапс в blank).', loss)

def log_eval_loss_cer_table_from_history(log_history: List[Dict[str, Any]]) -> None:
    rows: List[Tuple[Any, Any, Any, Any]] = []
    for row in log_history:
        if 'eval_loss' in row and 'eval_cer' in row:
            rows.append((row.get('step'), row['eval_loss'], row['eval_cer'], row.get('eval_wer')))
    if not rows:
        logger.info('CTC diag: в log_history нет пар (eval_loss, eval_cer) — evaluate ещё не вызывался или другие ключи.')
        return
    logger.info('CTC diag: история evaluate (step | eval_loss | eval_cer | eval_wer)')
    for step, loss_v, cer_v, wer_v in rows:
        logger.info('  %s | eval_loss=%s | eval_cer=%s | eval_wer=%s', step, loss_v, f'{cer_v:.6f}' if isinstance(cer_v, (int, float)) else cer_v, f'{wer_v:.6f}' if isinstance(wer_v, (int, float)) else wer_v)

def run_w2v_ctc_greedy_diagnostics(trainer: Trainer, eval_dataset: Dataset, processor: Any, *, eval_samples_cap: Optional[int]=None, sample_batches: int=1, histogram_max_samples: int=512, histogram_top_k: int=20, artifact_json: Optional[Path]=None, run_evaluate: bool=True) -> Dict[str, Any]:
    tok = processor.tokenizer
    blank_id = int(tok.pad_token_id)
    n_full = len(eval_dataset)
    if eval_samples_cap is not None:
        n_use = min(int(eval_samples_cap), n_full)
        ds_eval = eval_dataset.select(range(n_use))
    else:
        n_use = n_full
        ds_eval = eval_dataset
    summary: Dict[str, Any] = {'ctc_blank_pad_token_id': blank_id, 'eval_samples_used': n_use, 'eval_samples_full_split': n_full}
    was_training = trainer.model.training
    trainer.model.eval()
    metrics: Dict[str, float] = {}
    if run_evaluate:
        metrics = trainer.evaluate(ds_eval)
        summary['evaluate_metrics'] = dict(metrics)
        logger.info('CTC diag evaluate: eval_loss=%s eval_cer=%s eval_wer=%s (samples=%s/%s)', metrics.get('eval_loss'), metrics.get('eval_cer'), metrics.get('eval_wer'), n_use, n_full)
    eval_dl = trainer.get_eval_dataloader(ds_eval)
    batch_ix = 0
    all_empty = 0
    all_rows = 0
    ...
    for batch in eval_dl:
        ...
        if torch.isnan(logits).any():
            logger.error('CTC diag: NaN в logits — утечка/overflow (часто fp16).')
        pred_ids = logits.argmax(dim=-1).detach().cpu().numpy()
        labels_np = batch['labels'].detach().cpu().numpy()
        labels_fixed = labels_np.copy()
        labels_fixed[labels_fixed == -100] = blank_id
        pred_str = processor.batch_decode(pred_ids)
        label_str = processor.batch_decode(labels_fixed, group_tokens=False)
        bsz = len(pred_str)
        for i in range(bsz):
            pr, lb = (pred_str[i], label_str[i])
            sample_records.append({'label': lb, 'pred': pr})
            logger.info('CTC diag batch=%s item=%s | LABEL=%r | PRED=%r | empty_pred=%s', batch_ix, i, lb, pr, pr == '')
            all_rows += 1
            if pr == '':
                all_empty += 1
        batch_ix += 1
        if batch_ix >= max(1, sample_batches):
            break
    summary['sample_strings'] = sample_records[:max(1, sample_batches) * 16]
    summary['first_batches_empty_pred_frac'] = float(all_empty) / float(max(1, all_rows))
    n_hist = min(int(histogram_max_samples), len(ds_eval))
    if n_hist > 0:
        sub = ds_eval.select(range(n_hist))
        logits_np, _ = predict_logits_and_refs(trainer, sub, processor, need_full_logits=True)
        arg_ids = np.argmax(logits_np, axis=-1).ravel()
        V = int(logits_np.shape[-1])
        counts = np.bincount(arg_ids.astype(np.int64, copy=False), minlength=V)
        top_idx = np.argsort(counts)[::-1][:max(1, int(histogram_top_k))]
        hist_rows: List[Dict[str, Any]] = []
        total_frames = int(arg_ids.size)
        for idx in top_idx:
            idx_i = int(idx)
            c = int(counts[idx_i])
            tkn = tok.convert_ids_to_tokens(idx_i)
            hist_rows.append({'id': idx_i, 'token': '' if tkn is None else str(tkn), 'count': c, 'frac_of_frames': float(c) / float(max(1, total_frames)), 'is_ctc_blank_pad_id': idx_i == blank_id})
        summary['argmax_histogram_top'] = hist_rows
        summary['argmax_histogram_num_frames'] = total_frames
        logger.info('CTC diag argmax по кадрам (первые %s клипов, %s кадров), top-%s токенов (CTC blank = pad_token_id = %s):', n_hist, total_frames, histogram_top_k, blank_id)
        for row in hist_rows:
            mark = '  *** blank' if row['is_ctc_blank_pad_id'] else ''
            logger.info('  id=%s count=%s frac=%.4f token=%r%s', row['id'], row['count'], row['frac_of_frames'], row['token'], mark)
    if was_training:
        trainer.model.train()
    else:
        trainer.model.eval()
    if artifact_json is not None:
        ...
        artifact_json.write_text(json.dumps(summary, indent=2, ensure_ascii=False, default=str), encoding='utf-8')
        logger.info('CTC diag JSON: %s', artifact_json)
    return summary
...
def decode_beam_batch_wav2vec2(logits: np.ndarray, decoder: Any, beam_width: int, processor: Wav2Vec2Processor) -> List[str]:
    ...
    perm_arr = np.array([blank_id] + [i for i in range(n_vocab) if i != blank_id], dtype=np.int64)
    logits_perm = logits[..., perm_arr]
    return decode_beam_batch_wav2vec2_already_blank_first(logits_perm, decoder, beam_width, processor, perm_arr)

def decode_beam_batch_wav2vec2_already_blank_first(logits_blank_first: np.ndarray, decoder: Any, beam_width: int, processor: Wav2Vec2Processor, perm_to_original_token_id: np.ndarray) -> List[str]:
    n_vocab = int(logits_blank_first.shape[-1])
    base = SURROGATE_CTC_LABEL_BASE
    perm = perm_to_original_token_id
    out: List[str] = []
    for i in range(logits_blank_first.shape[0]):
        lp = _numpy_log_softmax(logits_blank_first[i].astype(np.float64, copy=False), axis=-1)
...
def export_metrics_excel(path: Path, greedy: Dict[str, Tuple[float, float]], meta: Dict[str, Any], beam: Optional[Dict[str, Tuple[float, float]]]=None) -> None:
    import pandas as pd
    include_beam = bool(beam) and any((split in beam and beam[split] is not None for split in ('train', 'test')))
    rows = []
    for split in ('train', 'test'):
        g = greedy.get(split, (np.nan, np.nan))
        row: Dict[str, Any] = {'split': split, 'greedy_WER': g[0], 'greedy_CER': g[1]}
        if include_beam:
    ...
    overview_row: Dict[str, Any] = {'train_greedy_WER': tr_g[0], 'train_greedy_CER': tr_g[1], 'test_greedy_WER': ts_g[0], 'test_greedy_CER': ts_g[1]}
    if include_beam:
        tr_b = beam.get('train', (np.nan, np.nan))
        ts_b = beam.get('test', (np.nan, np.nan))
        overview_row['train_beam_WER'] = tr_b[0]
        overview_row['train_beam_CER'] = tr_b[1]
        overview_row['test_beam_WER'] = ts_b[0]
        overview_row['test_beam_CER'] = ts_b[1]
        overview_row['beam_width_final_eval'] = meta.get('beam_width_used')
    ...
    split_sheet = 'by_split_greedy_beam' if include_beam else 'by_split_greedy'
    with pd.ExcelWriter(path, engine='openpyxl') as w:
        df.to_excel(w, sheet_name=split_sheet, index=False)
