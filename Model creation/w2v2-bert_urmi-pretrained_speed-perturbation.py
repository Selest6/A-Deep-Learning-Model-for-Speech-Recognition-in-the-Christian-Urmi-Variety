...
def parse_pipeline_args() -> argparse.Namespace:
    ...
    p.add_argument('--output_dir', type=str, default='./w2v-bert-urmi-pipeline-out')
    p.add_argument('--train_dir', type=str, default=None)
    p.add_argument('--test_dir', type=str, default=None)
    p.add_argument('--base_model_name_or_path', type=str, default='facebook/w2v-bert-2.0')
    p.add_argument('--phase', choices=('both', 'pretrain_only', 'finetune_only'), default='both')
    p.add_argument('--pretrained_encoder_checkpoint', type=str, default=None)
    p.add_argument('--disable_wandb', action='store_true')
    p.add_argument('--skip_pretrain', action='store_true')
    p.add_argument('--skip_finetune', action='store_true')
    return p.parse_args()

def run_urmi_pipeline(cli: argparse.Namespace) -> None:
    out = Path(cli.output_dir).expanduser().resolve()
    pretrain_root = out / 'pretrain'
    finetune_root = out / 'finetune_experiment_2_augment_style'
    repo = Path(__file__).resolve().parent
    encoder_ckpt: Optional[Path] = None
    if cli.phase in ('both', 'pretrain_only') and (not cli.skip_pretrain):
        pretrain_root.mkdir(parents=True, exist_ok=True)
        pre_args = parse_args()
        pre_args.phase = 'pretrain_only'
        pre_args.output_dir = str(out)
        pre_args.base_model_name_or_path = cli.base_model_name_or_path
        pre_args.disable_wandb = cli.disable_wandb
        if cli.train_dir:
            pre_args.pretrain_audio_roots = [str((repo / '5h_urmi_pretraining').resolve()), str(Path(cli.train_dir).resolve() / 'audios')]
        encoder_ckpt = run_pretraining(pre_args, pretrain_root)
    if cli.phase == 'finetune_only':
        if not cli.pretrained_encoder_checkpoint:
            raise ValueError('--pretrained_encoder_checkpoint required for finetune_only')
        encoder_ckpt = Path(cli.pretrained_encoder_checkpoint).expanduser().resolve()
    elif cli.skip_pretrain or cli.phase == 'finetune_only':
        encoder_ckpt = out / 'pretrain' / 'best_pretrained_model'
    if cli.phase in ('both', 'finetune_only') and (not cli.skip_finetune):
        if encoder_ckpt is None or not Path(encoder_ckpt).is_dir():
            raise FileNotFoundError(f'Missing encoder checkpoint: {encoder_ckpt}')
        finetune_root.mkdir(parents=True, exist_ok=True)
        fp = build_parser()
        fp.set_defaults(model_name_or_path=cli.base_model_name_or_path, pretrained_encoder_dir=str(Path(encoder_ckpt).resolve()), train_dir=cli.train_dir or str(repo / '5h_train'), test_dir=cli.test_dir or str(repo / '5h_test'), output_dir=str(finetune_root), experiment_name='w2v2-bert_urmi-pretrained_speed-preturbation', train_augment_mode='speed', mask_time_prob=0.0, mask_feature_prob=0.0, speed_factors='0.9,1.1', speed_prob=0.5, add_adapter=True, ctc_zero_infinity=True, lr_scheduler_type='cosine', weight_decay=0.01, learning_rate=1.5e-05, warmup_ratio=0.1, per_device_train_batch_size=1, per_device_eval_batch_size=1, gradient_accumulation_steps=8, max_grad_norm=0.5, disable_wandb=cli.disable_wandb)
        for act in fp._actions:
            if act.dest in ('train_dir', 'test_dir'):
                act.required = False
        run_training(fp.parse_args([]))
    manifest: Dict[str, Any] = {'output_dir': str(out), 'pretrain_root': str(pretrain_root.resolve()), 'best_encoder': str((pretrain_root / 'best_pretrained_model').resolve()), 'finetune_out': str(finetune_root.resolve())}
    (out / 'pipeline_manifest.json').write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')
    logger.info('Pipeline done: %s', out)

def main() -> None:
    logging.basicConfig(level=logging.INFO)
    cli = parse_pipeline_args()
    run_urmi_pipeline(cli)
